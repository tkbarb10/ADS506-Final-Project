---
title: "Group 3 Final Report Appendix"
...
Interestingly, `tslm_log` is the best model by far on the AIC metric, however log transformations end up with forecasts that are small and have little variance, artificially inflating the log likelihood of fitting the data and resulting in low AIC scores.  Among the models with predictors and no transformations, `tslm_co2_nino`has the lowest AIC and is the second best performing on RMSE.
...
Among box-cox transformed models, `tslm_volcanic` has the lowest AIC.  All numbers are close though so any of these models would be viable
...
The model including features for volcanic activity and non-lagged variables for CO2 and ENSO is the best overall by any metric
...
The log transformation models perform the best on the validation data and among those, the `arima_volcano_basic` model has the lowest AIC. We saw a similar result with the TSLM models, volcanic activity seems to improve results. 
...
The `arima_volcano_basic` model selected AR(1), MA(1) and sMA(2) with a seasonal difference. The ACF and PACF plots still contain spikes at lag 3. Interestingly, the `arima_physics_robust` model contains a spike at lag 24 but not 3. There might be something in delaying the El Nino effect by a few months that explains the autocorrelation.
...
Not necessarily the best way to ascertain how important variables are, but informative nonetheless. ENSO related variables and seasonal ARIMA parameters seem to be the most important to modeling this type of data, which bears out in the results.
...
The top model by AIC was second best metric wise. `arima_physics_robust_decay_enso_3` is most likely the best model overall
...
Retraining the top models from each cohort on the same subset of data and comparing results
...
The TSLM models perform the best on the validation data across all metrics.  ETS models do the worst, however their results are better on the post-1957 data than they were on the entire dataset, suggesting that the later data is more representative of the recent trend.
...
A major downside of the TSLM models is the presence of autocorrelation.  ARIMA models seem to be able to account for most if not all of it.

```{r final-visual}

final_fc |>
  filter(.id %in% c(4, 5), .model %in% c("tslm_no_trend", "arima_physics_robust_decay_enso_3", "ets_additive")) |> 
  autoplot() +
  autolayer(predictor_modeling |> filter(year(Date) >= 2015), actual_temp) +
  facet_wrap(~ .model, ncol = 1) +
  theme_minimal()
...
The accuracy of TSLM and ARIMA is clear from visualizing the splits, however the presence of autocorrelation indicates that some version of an ARIMA model is the best choice for this dataset.

While the `arima_physics_robust_decay_enso_3` model had the best results, volcanic activity is impossible to predict and the radiative forcing equation used is a simplified version of the IPCC standard.  Because the other models are comparable, it was decided that one of the other ARIMA models would be a better choice.

We noticed ACF spikes at lags 12 and 24 for `arima_auto_10`, and auto-ARIMA was selecting either ARIMA(3,0,2)(1,1,0) with `enso_smooth_6` or ARIMA(2,0,2)(1,1,0) with `enso_smooth_3.` We added a seasonal MA term, which removed the spikes, and the seasonal AR term turned out to be non-significant, so it was removed. The AIC was slightly higher when using `enso_smooth_6.`  The final model chosen was ARIMA(3, 0, 2)(0, 1, 1) with 10yr lagged CO2 concentration and 6-month smoothed ENSO index as predictors.

# Scenario Forecasting

```{r scenario-forecasting}

# 2024 average monthly co2 emissions

current_baseline <- 3400921

# co2 concentration is best modeled using a 12 month lagged aggregate emissions variable and a 4 month lagged ENSO variable smoothed over 12 months
...
The ENSO index can't be appropriately modeled, so instead, to simulate 50 year forecasts, the ENSO cycles that occurred over various 50 yr time periods in the available data were replicated to be used to simulate different conditions.  Over all ~ 150 years of data, the average ENSO index was 0.0470, so 50yr slices of the data were taken that could simulate a higher, lower and similar ENSO index to model varying conditions

```{r scenarios}

scenario_forecasting <- function(growth_rate, years, enso_level) {
  
  stopifnot(enso_level %in% c(1, 2, 3))
  stopifnot(years > 0)
  stopifnot(as.numeric(growth_rate))
...
The scenario forecasting function takes as inputs the `growth_rate` as a percent, `years` to forecast, and the `enso_level` argument is an integer denoting which enso conditions the user wishes to simulate.  1 is for lower than average, 2 is for normal conditions and 3 is for higher than average.
...
The `zoo` library can be used to perform a spline regression on the annual estimates we replicated across monthly values to even out the resulting step curve.  Then the average seasonal component can be estimated from the true monthly observations, multiplied by a scale factor, then added back to the corresponding interpolated observation.  The results can be seen below
...
The volcanic events with a lower VEI than 4 are filtered out as their impact on the climate is most likely too minimal to significantly affect the global climate.  A dummy variable is created for the presence of the event, then an exponential decay formula extending over the course of 3 years was applied.  Overlapping events were summed
