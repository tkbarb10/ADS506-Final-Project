...
predictor_modeling_lags <- predictor_modeling |>
  mutate(
    # Example ENSO index, if you want a single factor:
    ENSO = case_when(
      el_nino == 1 ~ 1,
      la_nina == 1 ~ -1,
      TRUE ~ 0
    ),
    # Example smoothed ENSO (12-month rolling mean; requires tsibble index already set)
    enso_smooth = slider::slide_dbl(ENSO, mean, .before = 11, .complete = TRUE),
    # 10-month lag of CO₂
    co2_lag10 = dplyr::lag(co2_ppm, 10)
  ) |>
  drop_na()  # drop initial rows where lag/smoothing produce NA
