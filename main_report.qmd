...
Fourier terms for a simpler model, piece-wise to capture changes in trend at those points
...
mention lagged data?  Bring it up earlier?

```{r arima-data}

lagged_data <- read_rds("data/lagged_predictors.rds")

arima_data <- read_rds('data/lagged_nino_predictors.rds')

trn_data <- arima_data |>
  slice(1:(n() - 60))

valid_data <- arima_data |> 
  slice_tail(n = 60)
```
...
arima_fit <- trn_data |> 
  model(
    ...
    arima_all = ARIMA(actual_temp ~ co2_ppm + ch4 + TSI + el_nino + la_nina),
    arima_ppm_ten = ARIMA(actual_temp ~ lagged_ppm_ten),
    arima_lag_ten_nino = ARIMA(actual_temp ~ lagged_ppm_ten + el_nino + la_nina),
    arima_all_log = ARIMA(log(actual_temp) ~ co2_ppm + ch4 + TSI + el_nino + la_nina),
    arima_all_log_lag = ARIMA(log(actual_temp) ~ lagged_ppm_ten + ch4 + TSI + el_nino + la_nina)
  )
...
Using top models and parameter set ups for cv training
