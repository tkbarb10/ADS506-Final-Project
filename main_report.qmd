...
This dataset contains 2,109 monthly observations from 1850–2024, with global temperature anomalies and uncertainties, where **Monthly_Anomaly** is the primary usable series and most multi-year anomaly fields contain NA values except at the end of their smoothing windows.

***Do we want to add a visual here of the time series? Otherwise it's not really clear that there are year-to-year fluctuations or that the LOWESS-smoothed line clarifies the upward trend (I deleted it out)***

```{r kamran-rename}
climate_clean <- climate_raw |>
  janitor::clean_names() |>
  rename(
    no_smoothing  = monthly_anomaly,
    lowess_5      = five_year_anomaly
  )
```
...
```{r kamran-load-monthly}

gistemp_annual <- gistemp_monthly_raw |>
  rename(raw = land_ocean_temperature_index_c) |>
  separate(
    raw,
    into = c("year", "no_smoothing", "lowess_5"),
    sep = ","
  ) |>
  slice(-1) |>  # drop the header row "Year,No_Smoothing,Lowess(5)"
  mutate(across(everything(), as.numeric))

glimpse(gistemp_annual)
```
