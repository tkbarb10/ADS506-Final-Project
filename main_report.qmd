...
***Do we want to add a visual here of the time series? Otherwise it's not really clear that there are year-to-year fluctuations or that the LOWESS-smoothed line clarifies the upward trend***
...
***This is where it would be useful to use the monthly observations instead, so we could see the seasonal component***
...
λ ≈ 1.16 → little to no Box–Cox transformation needed; variance is already stable. Unitroot test suggests differencing may be needed
...
𝛌≈ 1.44, so a power transformation may be helpful
...
***Seems redundant to plot this twice? would it be better to just plot it once using the zoomed in version? And filter by 2010 so we get some of the pre-forecast observations***
...
***Might be good to comment on how wide the prediction intervals are too***
...
The metrics based on RMSE show that ARIMA does slightly better than ETA and both do much better than TSLM.  Comparing ARIMA to ETS visually, we see much more uncertainty in the ETS predictions.  Both models still seem to under shoot the peaks which is a strong indicator that other factors are at play that will need to be considered


### TSLM External Predictors

How far back accurate monthly data is available for the following predictors varies.  However for the main drivers of temperature, GHG's, the most accurate monthly estimates began in 1958.  So that will be the cutoff date for our training data.

**Need references here** According to ______ the main factors that influence temperature are GHG's, weather patterns, and solar irradiance.  To see which ones best serve our purpose, we can model the following and compare performance
...
The earliest data that could be found on historical el nino/la nina events dated back to 1896, so the regressor data will extend from 1896 to Sept 2025
...
tslm_regressor_fit <- cv_trn |> 
  model(
    ...
    tslm_co2_nino_lag = TSLM(actual_temp ~ trend() + season() + el_nino + la_nina + co2_lag10)
  )
...
tslm_regressor_fit |> 
  ...
  kable(digits = 4)
```
Training metrics show that adding predictors improve model results over baseline.  This could be due to overfitting
...
TSLM with a box_cox transformation and all of the predictors performed the best on the validation data.  The `enso_smooth` variable also performed better than either `ENSO` or the nino dummy variables.  These results indicate that other model combinations are worth exploring.  Interesting to note that adding `ch4_ppb` actually reduced model performance compared to just using `co2_ppm`

```{r tslm-round-two}
tslm_second_fit <- cv_trn |> 
  model(
    tslm_all_lag_box = TSLM(box_cox(actual_temp, lambda = 1.5) ~ trend() + season() + el_nino + la_nina + co2_lag10 + n2o_lag10 + ch4_lag10),
    tslm_nino_five_lag = TSLM(actual_temp ~ trend() + season() + el_nino + la_nina + co2_lag5),
    tslm_enso_co2_lag = TSLM(actual_temp ~ trend() + season() + ENSO + co2_lag10),
    tslm_enso_smooth_lag_all = TSLM(actual_temp ~ trend() + season() + enso_smooth_12 + co2_lag10 + n2o_lag10 + ch4_lag10),
    tslm_all_lag = TSLM(actual_temp ~ trend() + season() + el_nino + la_nina + co2_lag10 + n2o_lag10 + ch4_lag10),
    tslm_nino_50_lag = TSLM(box_cox(actual_temp, lambda = 1.5) ~ trend() + season() + el_nino + la_nina + co2_lag50),
...
The target variable undergoing a box_cox transformation using the smoothed enso index over 6 months and a 50 year lag in CO2 concentration performs the best out of all the models by a fair margin
...
tslm_second_fit |> 
  ...
  report()
```
And we can see from the report that all the predictors used are significant except trend.  It could be that smoothing out the enso index and the 50 yr lag of co2_ppm captures most of the trend component

```{r tslm-best-model}
tslm_best_fit <- cv_trn |> 
  model(
    tslm_enso_6 = TSLM(box_cox(actual_temp, lambda = 1.5) ~ trend() + season() + enso_smooth_6 + co2_lag50),
    tslm_enso_12 = TSLM(box_cox(actual_temp, lambda = 1.5) ~ trend() + season() + enso_smooth_12 + co2_lag50),
...
tslm_best_fc |> 
  accuracy(cv_valid) |> 
  group_by(.model) |>
  summarise(across(.cols = c(ME, RMSE, MAE, MAPE), .fns = mean)) |> 
  arrange(RMSE) |> 
  kable(digits = 4)
```
...
tslm_fc |>
  ...
  filter(.id %in% c(5, 6), .model %in% c("tslm_no_trend", "tslm_enso_smooth_lag", "tslm_nino_50_lag", "tslm_co2_nino_lag", "tslm_box_cox")) |> 
  autoplot() + 
  autolayer(predictor_modeling |> filter(year(Date) >= 2010), actual_temp) + 
  facet_grid(.model ~ .id) + 
  theme_minimal()
```

Comparing best models to the under-performing ones, we see that adding predictors, especially ones that are lagged, closes the gap in the increasing trend that other models seem to miss. 
...
arima_fit <- arima_trn |> 
  model(
    ...
    arima_lag_30 = ARIMA(actual_temp ~ co2_lag30 + PDQ(D=1)),
...
arima_fc |> 
  ...
  kable(digits = 4)
```
The arima_physics_robust model is the best overall by any metric.  the other best models by AICc are middle of the pack on the validation metrics **include a more robust explanation of why a delay in the enso index and why co2 radiative forcing on it's own aren't strong signals**
...
The arima_physics_robust model selected AR(2), MA(1) and sAR(2) with a seasonal difference.  The ACF and PACF plots still contain spikes at lag 24.  Interestingly, arima_enso and arima_enso_smooth don't have autocorrelation spikes at lag 24, only at lag 3.  Maybe some combination of those with rf_co2 or co2 would be worth exploring
...
arima_second_fit <- cv_trn |> 
  model(
    ...
    arima_auto_50_enso_6 = ARIMA(log(actual_temp) ~ enso_smooth_6 + co2_lag50 + PDQ(D = 1)),
    arima_robust_enso_6 = ARIMA(log(actual_temp) ~ rf_co2 + volcano_linear + enso_smooth_6 + PDQ(D = 1)), 
    arima_co2_50_volcano = ARIMA(log(actual_temp) ~ co2_lag50 + volcano_linear + PDQ(D = 1)),
    arima_co2_30_nino = ARIMA(actual_temp ~ co2_lag30 + el_nino + la_nina + PDQ(D = 1)),
    arima_co2_30_nino_tsi = ARIMA(actual_temp ~ co2_lag30 + el_nino + la_nina + TSI + PDQ(D = 1)),
...
Across multiple folds, the arima model utilizing the radiative forcing of CO2, volcanic activity and a 3 month smoothing of the enso index performs the best on most metrics other than MPE and ME.
...
The radiative forcing model was also the best model in terms of AICc.  It's also clear that a log transformation is necessary for this data to smooth out the variation in temperature.

Since the CO2 radiative forcing model performed best by RMSE on the validation data and it was the best model by AICc standards, that might be the best model to choose
...
No autocorrelation spikes in the residuals and the Ljung-Box test confirms that the residuals resemble white noise for the arima_auto_50 model

```{r arima_cv_visual}

arima_fc |>
  filter(.id %in% c(5, 6), .model %in% c("arima_auto_50", "arima_enso_log_co2", "arima_auto_50_enso_6", "arima_physics_robust")) |> 
  autoplot() +
