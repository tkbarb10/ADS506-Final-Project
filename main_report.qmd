...
```{r}
features(climate_ts, no_smoothing, unitroot_ndiffs)
```

The ndiffs() test indicates that the series requires one regular difference (d = 1) to achieve stationarity, consistent with the KPSS result showing strong trend non-stationarity. This confirms that an ARIMA model with d = 1 is appropriate for the anomaly series.

```{fit_arima <- climate_ts |>}
  model(ARIMA(no_smoothing))
```
...
The auto-ARIMA procedure selected an **ARIMA(1,1,3) with drift**, indicating that the series requires one difference to remove the long-term warming trend, while the AR(1) and multiple MA terms capture the strong year-to-year autocorrelation seen in the ACF and PACF. The positive drift term (0.0175) represents the **average yearly increase** in the global temperature anomaly, consistent with the long-term upward trend.
...
fc_arima |>
  autoplot(climate_ts) +
  labs(
    title = "ARIMA Forecast of Annual Global Temperature Anomalies",
    x = "Year",
    y = "Temperature Anomaly (°C)"
  ) +
  theme_minimal()
...
The ARIMA(1,1,3) forecast shows a persistent upward trajectory consistent with the historical warming trend, with anomalies likely reaching or exceeding **1.3–1.5°C** by mid-century. The prediction intervals widen over time, but their lower bounds remain well above the 20th-century baseline, reinforcing the robustness of the projected warming.
...
The diagnostic plots suggest that the ARIMA model has stripped out the trend and autocorrelation from the data, leaving behind only random year-to-year noise. With no remaining structure in the ACF and residuals centered on zero, the model passes the key adequacy checks.
...
# Comparing the best models with regressors added in on the same data split
tslm_regressor_fit <- cv_trn |> 
  model(
    ...
    tslm_co2_nino = TSLM(actual_temp ~ trend() + season() + el_nino + la_nina + co2),
    tslm_ch4_co2_nino = TSLM(actual_temp ~ trend() + season() + el_nino + la_nina + co2 + ch4),
    tslm_all = TSLM(actual_temp ~ trend() + season() + el_nino + la_nina + co2 + ch4 + TSI),
    tslm_all_box = TSLM(box_cox(actual_temp, lambda = 1.5) ~ trend() + season() + el_nino + la_nina + co2 + ch4 + TSI)
  )
...
# check report of best model fit on most data
tslm_regressor_fit |> 
  select(tslm_all_box) |>
  tail(n = 1) |> 
  report()
...
Best model according to the training data is the tslm modeling with the box cox transformation and adding in all predictors.  Every variable shows as significant

```{r regressor-forecasts}
...
tslm_fc |> 
  filter(.id %in% c(12, 13), .model %in% c("tslm_all_box", "tslm_all", "tslm_ch4_co2_nino", "tslm_co2_nino")) |> 
  autoplot() + 
  autolayer(tslm_model |> filter(year(Date) >= 2010), actual_temp) + 
  facet_grid(.model ~ .id) + 
  theme_minimal()
```

Results hold up on the validation data
